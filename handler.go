...
type Locations struct {
	Index []struct {
		LocationCities []string `json:"locations"`
	}
}
...
var (
	...
	loc, errorOfLocation     = GetLocation()
)

func (u *Album) getLoc(n int) {
	t := *loc
	u.Cities = t.Index[n].LocationCities
	for i, s := range u.Cities {
		if strings.Contains(s, "_") {
			temp := strings.Title(strings.Replace(s, "_", " ", -1))
			u.Cities[i] = temp
		} else {
			temp := strings.Title(s)
			u.Cities[i] = temp
		}
	}
}
